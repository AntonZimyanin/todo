...
from todo.api.action.todo import _select_all_todo, _create_todo, _update_todo, _delete_todo
...
async def update(
    ...
    update_todo_id = _update_todo(todo_id=todo_id, session=db_session, params={"is_complete": is_complete})
...
async def delete(todo_id: int = Path(..., description="todo id"), db_session: Session = Depends(get_db)):

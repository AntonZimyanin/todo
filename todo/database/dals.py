...
class TodoDAL: 
    ...
    def update_todo(self, todo_id: int, **kwargs) -> Union[int, None]: 
        query = (
            update(Todo).where(Todo.id == todo_id).values(kwargs).returning(Todo.id)
        )
        res = self.db_session.execute(query)
        update_todo_row = res.fetchone()
        print("!!!!!!!!!!!!", update_todo_row)
    ...
    def select_all(self) -> Union[list, EmptyList]:
        ...
        todo_list = [todo for todo_tuple in todo_tuple_list \
                      for todo in todo_tuple]
